...
6. 事件检测只用指定 CondParams。
   ...
   - 战斗开始：`BattleStartCondParams`。
   - 战斗结束：`BattleEndCondParams`。
...
补充核对来源：本地 `E:\ff14\AEAssistCNVersion\AEAssist\AEAssist.dll`。
...
### BattleStartCondParams / BattleEndCondParams

命名空间：`AEAssist.CombatRoutine.Trigger`

```csharp
public class BattleStartCondParams : ITriggerCondParams;
public class BattleEndCondParams : ITriggerCondParams;
```

用途：

- 只用它们检测战斗生命周期。
- `BattleStartCondParams` 可用于重置脚本实例状态、清理 pending 机制、清理 `DebugPoint`。
- `BattleEndCondParams` 可用于重置脚本实例状态、取消延迟任务有效性、清理 `DebugPoint`。
- 不从这两个事件里读取机制 ID、坐标、目标、玩家或职能；需要机制数据时继续用对应机制事件。
...
- 场地边界按机制固定，例如菱形场用 `Abs(x - center.X) + Abs(z - center.Z)` 判断。
- 没找到安全点时返回等待或保持现逻辑，不写随机点、职业顺序点或旧逻辑兜底。
...
```csharp
distance <= margin => dangerous;
distance <= radius + margin;
Abs(NormalizeAngle(pointAngle - rotation)) <= totalRadian / 2 + edgeMargin;
edgeMargin = Asin(Min(1, margin / distance));
```
